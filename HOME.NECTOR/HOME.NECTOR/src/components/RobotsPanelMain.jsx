...
const RobotsPanelMain = ({ onCancelSelection, selectedRobot }) => {
  ...
  const toggleState = (stateKey, connection, commandOn, commandOff) => {
    ...
    setRobotStates((prevStates) => {
      ...
      console.log(
        newState ? `Turning ON ${stateKey}` : `Turning OFF ${stateKey}`
      );
      return {
        ...prevStates,
  ...
  const defaultCamBtns = useMemo(
    () => (robotCameraViewRef, connection, camera) =>
      [
        {
          src: "closeConn.png",
          event: () => {
        ...
        },
        {
          ...
          event: () => toggleState("led", connection, "LED_ON", "LED_OFF"), // Przełączanie LED
        },
      ],
    []
  );
  ...
  const connectToRobot = () => {
    if (selectedRobotDevice) {
      ...
      ws.onclose = () => {
        console.log(
          `Robot connection closed with IP: ${selectedRobotDevice.IP_Address}`
        );
      ...
      ws.onopen = () => {
        console.log(
          `Robot connection open with IP: ${selectedRobotDevice.IP_Address}`
        );
        setRobotConn(ws);
      };
      ...
      ws.onerror = () => {
        console.log(
          `Robot connection error with IP: ${selectedRobotDevice.IP_Address}`
        );
      };
    ...
    <div className="robotsPanelMain">
      <div className="robotControlPanel">
        <div className="robotDataPanelsHolder">
          ...
          />
          <div className="w-[90%] h-[150px] bg-pink-400 rounded-[15px] shrink-0">
            Microphone data
          </div>
          <div className="w-[90%] h-[150px] bg-pink-400 rounded-[15px] shrink-0">
            Distance sensor
        ...
        <div className="robotButtonsHolder">
          {robotConn &&
            defaultCamBtns(robotCameraViewRef, robotConn, "camera").map(
              (element, index) => renderCameraBtn(element, index, robotConn)
            )}
        </div>
      </div>
      <div className="robot3DModelAndBtns">
        ...
        <div className="flex h-[20%] justify-evenly">
          ...
          <button
            className="robotCancelBtn"
            onClick={() => {
              setSelectedRobotDevice(null);
              onCancelSelection();
            }}
          >

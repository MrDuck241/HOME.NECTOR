  ...
  const renderCameraBox = (element, index) => {
    return (
      <button
        ...
        key={index}
        onClick={() => onCamDeviceClick(element)}
      >
        {element.Device_Name}
      </button>
    );
      ...
      {showSelectDeviceViewID && (
        <div className="w-[100vw] h-[100vh] bg-transparent backdrop-blur-md fixed top-0 left-0 z-[9999]">
          <div className="w-[25vw] h-[30vh] bg-gray-700 relative top-2/4 left-2/4 -translate-x-2/4 -translate-y-2/4 rounded-[12px] border-solid border-2 border-gray-400 flex flex-col justify-between">
            <div className="w-[100%]">
              <button
                className="closeSelectViewBtn"
                onClick={() => setShowSelectDeviceViewID(false)}
              ></button>
            </div>
            <div className="w-[100%] flex flex-row  justify-evenly mb-[20px]">
              <button
                type="button"
                className="selectCamViewBtn"
                onClick={() => {
                  onSelectCamDevice(1, bufforCamDevice);
                }}
              >
                Camera View Box 1
              </button>
              <button
                ...
                className="selectCamViewBtn"
                onClick={() => {
                  onSelectCamDevice(2, bufforCamDevice);
                }}
              >
                Camera View Box 2
              </button>
      ...
      <div className="camerasPanelNav">
        <div className="cameraDevicesHolder">
          {loading ? (
            ...
            fetchedCameras.map((element, index) =>
        ...
        <div className="navControlBtnsHolder">
          <div className="viewCamera3DModel">{/* Do zrobienia */}</div>
